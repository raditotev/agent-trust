...
def _session_factory(session_mock):
    """Return a callable that always yields a fresh context manager over session_mock."""
    def _factory():
...
def _make_session_for_agent(agent: MagicMock | None, trust_score: MagicMock | None = None):
    """Build a mock DB session. First execute → agent, second → trust score."""
    session = MagicMock()
    call_count = [0]
...
async def test_check_trust_uses_cache():
    """Cache hit → returns cached data without calling DB for score."""
    cached_data = {
        "agent_id": _AGENT_A,
        "score_type": "overall",
        "score": 0.85,
    ...
    # Session only needs to handle the agent existence check (1 call)
    agent = _make_agent(_AGENT_A)
    ...
    # setex should NOT be called since we hit cache
...
async def test_get_score_breakdown_returns_all_types():
    ...
    # Session: 1 agent check + 4 score lookups = 5 calls
    session = MagicMock()
    call_count = [0]
...
async def test_compare_agents_ranked():
    ...
    # compare_agents calls get_session() twice per agent (agent check + score lookup)
    # We need one shared session that can handle all 4 execute() calls in order:
    # call 0: agent_a existence check
    # call 1: agent_a score lookup (in _get_or_compute_score)
    # call 2: agent_b existence check
    # call 3: agent_b score lookup (in _get_or_compute_score)
    session = MagicMock()
    call_count = [0]
    ...
    async def execute(query):
        ...
        n = call_count[0]
        call_count[0] += 1
        result.scalar_one_or_none = MagicMock(return_value=responses[n])
        return result
...
def _make_computed_trust_score(
    agent_id: str,
    score_type: str = "overall",
    score: float = 0.75,
    confidence: float = 0.8,
    ...
    """Return a TrustScore-like object as returned by ScoreComputation.compute."""
    ts = MagicMock()
    ts.agent_id = uuid.UUID(agent_id)
    ts.score_type = score_type
    ts.score = score
...
def _make_session_for_agent(agent: MagicMock | None, trust_score: MagicMock | None = None):
    """Build a mock DB session that returns the given agent and optional trust score."""
    session = MagicMock()
    call_count = [0]
    ...
    async def execute(query):
        ...
        if n == 0:
            ...
            # Second call: trust score lookup
            result.scalar_one_or_none = MagicMock(return_value=trust_score)
        return result
...
async def test_check_trust_basic():
    ...
    with (
        patch("agent_trust.tools.scoring.get_session", return_value=_fake_session_ctx(session)),
        patch("agent_trust.tools.scoring.get_redis", return_value=AsyncMock(return_value=redis_mock)),
    ):
        # Patch get_redis as a coroutine returning the mock
        with patch("agent_trust.tools.scoring.get_redis", new=AsyncMock(return_value=redis_mock)):
            result = await check_trust(agent_id=_AGENT_A)
...
async def test_check_trust_agent_not_found():
    ...
    with patch("agent_trust.tools.scoring.get_session", return_value=_fake_session_ctx(session)):
        result = await check_trust(agent_id=str(uuid.uuid4()))
...
@pytest.mark.asyncio
async def test_check_trust_uses_cache():
    """Second call: Redis cache hit returns data without hitting DB."""
    cached_data = {
        "agent_id": _AGENT_A,
        "score_type": "overall",
        "score": 0.85,
    ...
    agent = _make_agent(_AGENT_A)
    # Session only needed for agent existence check (first call in check_trust)
    session = _make_session_for_agent(agent)
...
async def test_get_score_breakdown_returns_all_types():
    ...
    async def execute(query):
        ...
        if n == 0:
            ...
            st = ["overall", "reliability", "responsiveness", "honesty"][(n - 1) % 4]
            result.scalar_one_or_none = MagicMock(
                return_value=_make_trust_score(_AGENT_A, score_type=st)
            )
        return result
...
async def test_compare_agents_ranked():
    ...
    score_map = {
        _AGENT_A: _make_trust_score(_AGENT_A, score=0.9),
        _AGENT_B: _make_trust_score(_AGENT_B, score=0.4),
    }

    call_counters: dict[str, list[int]] = {_AGENT_A: [0], _AGENT_B: [0]}

    def _make_session_for(agent_id_str: str):
        session = MagicMock()
        c = call_counters[agent_id_str]
        ...
        async def execute(query):
            ...
            if n == 0:
                result.scalar_one_or_none = MagicMock(
                    return_value=agent_a if agent_id_str == _AGENT_A else agent_b
                )
            else:
                result.scalar_one_or_none = MagicMock(return_value=score_map[agent_id_str])
            return result
    ...
    sessions = [_make_session_for(_AGENT_A), _make_session_for(_AGENT_B)]
    ...
    with (
        patch("agent_trust.tools.scoring.get_session", side_effect=lambda: _session_factory()),
        patch("agent_trust.tools.scoring.get_redis", new=AsyncMock(return_value=redis_mock)),
    ):
        result = await compare_agents(agent_ids=[_AGENT_A, _AGENT_B])
